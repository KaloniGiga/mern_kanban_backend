...
export const createList = async (
  ...
  try {
    ...
    const positionTaken = board.lists.find(
      (list: any) => list.position === position
    );
...
export const getLists = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    const cards = await Card.find({
      ...
      .lean()
      .select(
        "_id name description position listId isComplete expireDate coverImage labels comments creator members color"
      )
      .populate({
        path: "labels",
    ...
    const finalLists = board.lists.map((list: any) => {
      const finalCards = cards.filter(
        (card: any) => card.listId.toString() === list._id.toString()
      );
...
export const deleteList = async (
  ...
  try {
    ...
    const workspaceMember = board?.workspaceId.members.find(
      (member: any) => member.memberId.toString() === req.user._id.toString()
    )?.role;
    ...
    board.lists = board.lists.filter(
      (list: any) => list._id.toString() !== list._id.toString()
    );
...
export const updateList = async (
  ...
  try {
    ...
    if (!listId) {
      return next(new ErrorHandler(400, "ListId is requried"));
    } else if (!mongoose.isValidObjectId(listId)) {
      return next(new ErrorHandler(400, "Invalid listid"));
    }
    ...
    if (!name) {
      ...
      return next(
        new ErrorHandler(400, "Name must not be greater than 50 characters")
      );
    }

    console.log("Listid validated");

    const list = await List.findOne({ _id: listId }).select("_id name boardId");
